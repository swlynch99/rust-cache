...
var $605f5fa259db9fdc$exports = {};
$605f5fa259db9fdc$exports = new URL("logger.d8789bc8.js", "file:" + __filename).toString();


var $3e859e4793aca73c$exports = {};
$3e859e4793aca73c$exports = new URL("constants.94590164.js", "file:" + __filename).toString();


/**
 * Deserializes the state
 */ function $b07aeee75d63537c$var$deserializeState(serializedState) {
    try {
...
module.exports.deserializeState = $b07aeee75d63537c$var$deserializeState;
function $b07aeee75d63537c$var$setStateError(inputs) {
...
function $b07aeee75d63537c$var$appendReadableErrorMessage(currentMessage, innerMessage) {
    let message = currentMessage;
    if (message.slice(-1) !== ".") message = message + ".";
    return message + " " + innerMessage;
}
function $b07aeee75d63537c$var$simplifyError(err) {
    ...
    while(curErr.innererror){
        ...
        code = curErr.code;
        message = $b07aeee75d63537c$var$appendReadableErrorMessage(message, curErr.message);
    }
    return {
        code: code,
        message: message
...
function $b07aeee75d63537c$var$processOperationStatus(result) {
        ...
        case "failed":
            {
                ...
                let postfix = "";
                if (err) {
                    const { code: code, message: message } = $b07aeee75d63537c$var$simplifyError(err);
                    postfix = `. ${code}. ${message}`;
                }
                const errStr = `The long-running operation has failed${postfix}`;
                stateProxy.setError(state, new Error(errStr));
                stateProxy.setFailed(state);
                $605f5fa259db9fdc$exports.logger.warning(errStr);
                break;
            }
        case "canceled":
    ...
    if ((isDone === null || isDone === void 0 ? void 0 : isDone(response, state)) || isDone === undefined && [
        ...
        "failed"
    ]).includes(status)) stateProxy.setResult(state, $b07aeee75d63537c$var$buildResult({
        response: response,
        state: state,
        processResult: processResult
    }));
}
function $b07aeee75d63537c$var$buildResult(inputs) {
    const { processResult: processResult, response: response, state: state } = inputs;
    return processResult ? processResult(response, state) : response;
}
/**
 * Initiates the long-running operation.
 */ async function $b07aeee75d63537c$var$initOperation(inputs) {
    ...
    $605f5fa259db9fdc$exports.logger.verbose(`LRO: Operation description:`, config);
    ...
    $b07aeee75d63537c$var$processOperationStatus({
        state: state,
        status: status,
...
module.exports.initOperation = $b07aeee75d63537c$var$initOperation;
async function $b07aeee75d63537c$var$pollOperationHelper(inputs) {
    ...
    const response = await poll(operationLocation, options).catch($b07aeee75d63537c$var$setStateError({
        state: state,
        stateProxy: stateProxy,
        isOperationError: isOperationError
    }));
    const status = getOperationStatus(response, state);
    $605f5fa259db9fdc$exports.logger.verbose(`LRO: Status:\n\tPolling from: ${state.config.operationLocation}\n\tOperation status: ${status}\n\tPolling status: ${$3e859e4793aca73c$exports.terminalStates.includes(status) ? "Stopped" : "Running"}`);
    if (status === "succeeded") {
        const resourceLocation = getResourceLocation(response, state);
        if (resourceLocation !== undefined) return {
            response: await poll(resourceLocation).catch($b07aeee75d63537c$var$setStateError({
                state: state,
                stateProxy: stateProxy,
                isOperationError: isOperationError
            })),
...
/** Polls the long-running operation. */ async function $b07aeee75d63537c$var$pollOperation(inputs) {
    ...
    if (operationLocation !== undefined) {
        const { response: response, status: status } = await $b07aeee75d63537c$var$pollOperationHelper({
            poll: poll,
        ...
        if (!$3e859e4793aca73c$exports.terminalStates.includes(status)) {
...
module.exports.pollOperation = $b07aeee75d63537c$var$pollOperation;
