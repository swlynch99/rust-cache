...
const $2a94cec507371667$var$CollectionFormatToDelimiterMap = {
    CSV: ",",
    SSV: " ",
    Multi: "Multi",
    TSV: "\t",
...
function $2a94cec507371667$var$getRequestUrl(baseUri, operationSpec, operationArguments, fallbackObject) {
    const urlReplacements = $2a94cec507371667$var$calculateUrlReplacements(operationSpec, operationArguments, fallbackObject);
    let isAbsolutePath = false;
    let requestUrl = $2a94cec507371667$var$replaceAll(baseUri, urlReplacements);
    if (operationSpec.path) {
        let path = $2a94cec507371667$var$replaceAll(operationSpec.path, urlReplacements);
        ...
        // ignore the baseUri.
        if ($2a94cec507371667$var$isAbsoluteUrl(path)) {
            requestUrl = path;
            isAbsolutePath = true;
        } else requestUrl = $2a94cec507371667$var$appendPath(requestUrl, path);
    }
    const { queryParams: queryParams, sequenceParams: sequenceParams } = $2a94cec507371667$var$calculateQueryParameters(operationSpec, operationArguments, fallbackObject);
    /**
     * Notice that this call sets the `noOverwrite` parameter to true if the `requestUrl`
     * is an absolute path. This ensures that existing query parameter values in `requestUrl`
     * do not get overwritten. On the other hand when `requestUrl` is not absolute path, it
     * is still being built so there is nothing to overwrite.
     */ requestUrl = $2a94cec507371667$var$appendQueryParams(requestUrl, queryParams, sequenceParams, isAbsolutePath);
    return requestUrl;
}
module.exports.getRequestUrl = $2a94cec507371667$var$getRequestUrl;
function $2a94cec507371667$var$replaceAll(input, replacements) {
    let result = input;
    for (const [searchValue, replaceValue] of replacements)result = result.split(searchValue).join(replaceValue);
    return result;
}
function $2a94cec507371667$var$calculateUrlReplacements(operationSpec, operationArguments, fallbackObject) {
    var _a;
    ...
    if ((_a = operationSpec.urlParameters) === null || _a === void 0 ? void 0 : _a.length) for (const urlParameter of operationSpec.urlParameters){
        let urlParameterValue = (0, $bdQPg.getOperationArgumentValueFromParameter)(operationArguments, urlParameter, fallbackObject);
        const parameterPathString = (0, $kddm6.getPathStringFromParameter)(urlParameter);
...
function $2a94cec507371667$var$isAbsoluteUrl(url) {
    return url.includes("://");
}
function $2a94cec507371667$var$appendPath(url, pathToAppend) {
...
function $2a94cec507371667$var$calculateQueryParameters(operationSpec, operationArguments, fallbackObject) {
    var _a;
    const result = new Map();
    ...
    if ((_a = operationSpec.queryParameters) === null || _a === void 0 ? void 0 : _a.length) for (const queryParameter of operationSpec.queryParameters){
        ...
        let queryParameterValue = (0, $bdQPg.getOperationArgumentValueFromParameter)(operationArguments, queryParameter, fallbackObject);
        if (queryParameterValue !== undefined && queryParameterValue !== null || queryParameter.mapper.required) {
            queryParameterValue = operationSpec.serializer.serialize(queryParameter.mapper, queryParameterValue, (0, $kddm6.getPathStringFromParameter)(queryParameter));
            const delimiter = queryParameter.collectionFormat ? $2a94cec507371667$var$CollectionFormatToDelimiterMap[queryParameter.collectionFormat] : "";
            ...
            result.set(queryParameter.mapper.serializedName || (0, $kddm6.getPathStringFromParameter)(queryParameter), queryParameterValue);
        }
    }
    return {
        queryParams: result,
...
function $2a94cec507371667$var$simpleParseQueryParams(queryString) {
    const result = new Map();
...
/** @internal */ function $2a94cec507371667$var$appendQueryParams(url, queryParams, sequenceParams, noOverwrite = false) {
    if (queryParams.size === 0) return url;
    const parsedUrl = new URL(url);
    // QUIRK: parsedUrl.searchParams will have their name/value pairs decoded, which
    // can change their meaning to the server, such as in the case of a SAS signature.
    // To avoid accidentally un-encoding a query param, we parse the key/values ourselves
    const combinedParams = $2a94cec507371667$var$simpleParseQueryParams(parsedUrl.search);
...
module.exports.appendQueryParams = $2a94cec507371667$var$appendQueryParams;


//# sourceMappingURL=urlHelpers.2cc968fa.js.map
