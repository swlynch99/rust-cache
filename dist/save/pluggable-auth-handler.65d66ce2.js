...
var $8XbdC$child_process = require("child_process");
var $8XbdC$fs = require("fs");
...
var $gnn5W = parcelRequire("gnn5W");


/**
 * A handler used to retrieve 3rd party token responses from user defined
 * executables and cached file output for the PluggableAuthClient class.
 */ class $e38bfb4e66ef0ae3$var$PluggableAuthHandler {
    /**
     * Instantiates a PluggableAuthHandler instance using the provided
     * PluggableAuthHandlerOptions object.
     */ constructor(options){
        if (!options.command) throw new Error('No command provided.');
        this.commandComponents = $e38bfb4e66ef0ae3$var$PluggableAuthHandler.parseCommand(options.command);
     ...
     */ retrieveResponseFromExecutable(envMap) {
        return new Promise((resolve, reject)=>{
            ...
            const child = $8XbdC$child_process.spawn(this.commandComponents[0], this.commandComponents.slice(1), {
                env: {
                    ...process.env,
            ...
            child.on('close', (code)=>{
                ...
                try {
                    ...
                    const response = new $gnn5W.ExecutableResponse(responseJson);
                    return resolve(response);
                } catch (error) {
                    if (error instanceof $gnn5W.ExecutableResponseError) return reject(error);
                    return reject(new $gnn5W.ExecutableResponseError(`The executable returned an invalid response: ${output}`));
                }
                else return reject(new $e7qqe.ExecutableError(output, code.toString()));
            });
        });
    }
    /**
     ...
     */ async retrieveCachedResponse() {
        ...
        let filePath;
        try {
            filePath = await $8XbdC$fs.promises.realpath(this.outputFile);
        } catch (_a) {
            // If file path cannot be resolved, return undefined.
            return undefined;
        }
        if (!(await $8XbdC$fs.promises.lstat(filePath)).isFile()) // If path does not lead to file, return undefined.
        return undefined;
        const responseString = await $8XbdC$fs.promises.readFile(filePath, {
            encoding: 'utf8'
        });
        ...
        try {
            ...
            if (response.isValid()) return new $gnn5W.ExecutableResponse(responseJson);
            return undefined;
        } catch (error) {
            if (error instanceof $gnn5W.ExecutableResponseError) throw error;
            throw new $gnn5W.ExecutableResponseError(`The output file contained an invalid response: ${responseString}`);
        }
    }
    /**
...
module.exports.PluggableAuthHandler = $e38bfb4e66ef0ae3$var$PluggableAuthHandler;


//# sourceMappingURL=pluggable-auth-handler.65d66ce2.js.map
